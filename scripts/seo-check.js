...
for(const f of files){const html=fs.readFileSync(f,'utf8'); const rel=path.relative(root,f); const url='/' + (rel==='index.html'?'':rel.replace(/index\.html$/,''));
 ...
 for(const m of html.matchAll(/<img\b[^>]*>/gi)){const tag=m[0]; if(!/\salt=/.test(tag)) errors.push(`${rel}: image missing alt: ${tag.slice(0,100)}`)}
}
